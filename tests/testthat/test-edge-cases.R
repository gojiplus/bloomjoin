...
test_that("complex NA patterns are handled correctly", {
  ...
  # NA matches NA on both sides here by construction, so the reference join is
  # many-to-many on purpose. Declaring it keeps dplyr from warning about a
  # relationship this test exists to exercise.
  expected_mixed <- inner_join(
    x_mixed_na, y_mixed_na,
...
test_that("join column name conflicts", {
  ...
  has_suffixed <- "value.y" %in% names(result_conflict)
  has_any_value <- length(grep("value", names(result_conflict))) >= 1
  expect_true(has_suffixed || has_any_value)
})
