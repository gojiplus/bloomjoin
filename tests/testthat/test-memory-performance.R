...
test_that("memory usage is reasonable for large datasets", {
  ...
  if (!is.na(bloom_mem$memory_mb)) {
    ...
    cat(
      "Memory usage - Bloom:", round(bloom_mem$memory_mb, 2),
      "MB vs Standard:", round(std_mem$memory_mb, 2), "MB\n"
    )
  }
})
...
test_that("performance scales well with dataset size", {
  ...
  for (size in sizes) {
    ...
    has_ratio <- !is.null(metadata) && !is.null(metadata$reduction_ratio)
    reduction <- if (has_ratio) metadata$reduction_ratio else 0
...
test_that("join type performance characteristics", {
  ...
  for (join_type in join_types) {
    # Bloom join
    bloom_timing <- time_operation(
      bloom_join(df_large, df_small, by = "id", type = join_type, verbose = TRUE)
    )
