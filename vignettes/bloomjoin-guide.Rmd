...
result2 <- bloom_join(large_x, small_y,
  by = "id",
...
create_test_scenario <- function(n_left, n_right, selectivity) {
  ...
  # Right table with controlled selectivity: `selectivity` is the share of
  # right-hand rows that match. The rest are ids the left table does not
  # contain, which is what a Bloom filter is meant to reject cheaply.
  n_matches <- round(n_right * selectivity)
  matching <- sample(seq_len(n_left), n_matches, replace = FALSE)
  non_matching <- seq(n_left + 1, length.out = n_right - n_matches)
  right_df <- tibble(
    id = sample(c(matching, non_matching)),
...
scenarios <- list(
  list(
    name = "Optimal: Large left, small right, low selectivity",
    n_left = 50000, n_right = 1000, selectivity = 0.02
  ),
  list(
    name = "Good: Medium datasets, moderate selectivity",
    n_left = 10000, n_right = 2000, selectivity = 0.10
  ),
  list(
    name = "Poor: Small datasets, high selectivity",
