...
ecommerce_example <- function() {
  ...
  orders <- data.frame(
    ...
    order_date = sample(
      seq(as.Date("2023-01-01"), as.Date("2024-01-01"), by = "day"),
      10000,
      replace = TRUE
    )
  )
  ...
  cat(
    "- Bloom time:", sprintf("%.4fs", bloom_time),
    "vs Standard:", sprintf("%.4fs", std_time), "\n\n"
  )

  list(bloom = vip_orders_bloom, standard = vip_orders_std, metadata = metadata)
}
...
multicolumn_example <- function() {
  ...
  start_time <- Sys.time()
  enriched_bloom <- bloom_join(
    inventory, catalog,
    by = c("product_sku", "warehouse_code"), verbose = TRUE
  )
  bloom_time <- as.numeric(Sys.time() - start_time)
  ...
  list(bloom = enriched_bloom, standard = enriched_std, metadata = metadata)
}
...
na_handling_example <- function() {
  ...
  list(bloom = visits_bloom, standard = visits_std)
}
